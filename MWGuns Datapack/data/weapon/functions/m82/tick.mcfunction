...
execute if entity @e[type=area_effect_cloud,tag=WUID,tag=passed,tag=reload,limit=1] run function weapon:m82/reload
...
execute as @s[tag=!reloading] at @s anchored eyes if score @e[type=area_effect_cloud,tag=WUID,tag=passed,limit=1] magazine matches 1.. unless score @s shoot_delay matches 1.. if score @s click matches 1.. run function weapon:m82/shoot
